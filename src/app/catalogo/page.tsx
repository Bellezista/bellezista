...
import { MaquinariaBusqueda } from "@/components/catalogo/MaquinariaBusqueda";

export const dynamic = "force-dynamic";
export const metadata = { title: "Maquinaria · Bellezista" };
...
const SELLOS = [
  {
    ...
    texto: "Mensajería interna: tus datos protegidos.",
  },
  {
    Icon: HeartHandshake,
    titulo: "Asesoramiento experto",
    texto: "Te ayudamos a elegir la mejor opción.",
  },
  {
    Icon: Users,
    titulo: "Compra directa entre profesionales",
    texto: "Sin intermediarios, trato directo con el vendedor.",
  },
];
...
export default async function CatalogoPage() {
  const anuncios = await getAnunciosMaquinaria();
  ...
  return (
    <div className="bg-white text-foreground">
      ...
      <section className="relative flex min-h-[420px] items-center overflow-hidden bg-[#efe9df] md:min-h-[480px]">
        <img
          src={`${BUCKET}/hero-maquinaria.jpg`}
          alt="Maquinaria de estética profesional"
        ...
        <div className="relative w-full px-6 py-12 md:px-14 md:py-16 lg:px-20">
          <div className="max-w-[42rem]">
            <h1 className="font-serif text-[2.3rem] leading-[1.08] text-foreground md:text-[3.2rem]">
              Maquinaria estética
              <br className="hidden md:inline" /> para profesionales
            </h1>
            <p className="mt-5 max-w-[34ch] text-base leading-relaxed text-muted-foreground md:text-[1.05rem]">
              Encuentra el equipo que impulsará tu negocio con la mejor tecnología
              y al mejor precio.
            </p>
      ...
      {/* BÚSQUEDA + RESULTADOS */}
      <MaquinariaBusqueda anuncios={anuncios} />
